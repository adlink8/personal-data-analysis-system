...
SQLITE_DB = ROOT / "统合模块" / "SQLite数据库" / "personal_system.sqlite"
...
def finalize_report(report: dict) -> dict:
    ...
    if not chroma["available"]:
        ...
        actions.append(
            "Chroma live check failed，当前向量库状态为 blocked；先恢复 127.0.0.1:8001 的 Chroma，再重跑 `python 统合模块/脚本/evaluate_vector_collections.py --write`。"
        )
    ...
    for name in ("personal_events", "conversation_turns"):
        ...
        if not info.get("exists"):
            ...
            rebuild_cmd = (
                "python 统合模块/脚本/build_vector_store.py --write"
                if name == "personal_events"
                else "python 统合模块/脚本/build_conversation_vector_store.py --write"
            )
        ...
        if info.get("count_match") is False:
            ...
            if name == "conversation_turns":
                actions.append(
                    "conversation_turns 与 conversation_summaries.json 不一致，需重跑 `python 统合模块/脚本/build_conversation_vector_store.py --write`。"
                )
            else:
                actions.append(
                    "personal_events 与当前上游事件计数不一致，需重跑 `python 统合模块/脚本/build_vector_store.py --write`。"
                )
    ...
    if isinstance(sqlite_turns, int) and sqlite_turns != expected_turns:
        ...
        actions.append(
            "SQLite conversation_turns_summary 已过期，需同步 `python 统合模块/脚本/build_triple_store.py --write --only sqlite`。"
        )
