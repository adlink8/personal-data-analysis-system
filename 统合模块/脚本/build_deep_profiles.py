...
INTEGRATED = ROOT / "统合模块"
INTEGRATED_ANALYSIS = INTEGRATED / "分析数据"
INTEGRATED_DB = INTEGRATED / "SQLite数据库" / "personal_system.sqlite"
        ...
        rows.extend(
            [
                ...
                },
                {
                    "source": source,
                    "flow_step": "3 分析数据",
                    "location": f"{source}/分析数据",
                    "description": "生成模块报告、模块画像、增长图和关注点统计。",
                    "record_count": source_counts[source],
                },
                {
                    "source": source,
                    "flow_step": "4 统合模块",
                    "location": "统合模块/SQLite数据库/personal_system.sqlite",
...
def render_module_html(source: str, metrics: list[dict], top_topics: list[dict], top_thinking: list[dict]) -> str:
    ...
    <section class="section-grid">
      <article id="summary" class="panel">
        ...
        <p class="lede">{esc(source)} 模块画像描述这个来源在个人数据系统中的信号角色：它贡献什么数据、增长是否稳定、关注点集中在哪里，以及对个人 AI 上下文有什么用。</p>
        <ul class="insight-list">
          <li><strong>模块定位：</strong>{esc(metric_lookup.get("主要含义", {}).get("note", "该模块提供个人数据系统的一个独立侧面。"))}</li>
          <li><strong>证据基础：</strong>画像来自本地 SQLite/CSV 的结构化结果，HTML 只做展示，不改动原始数据。</li>
      ...
      <article id="growth" class="panel">
        ...
        <img class="chart-img" src="模块画像_数据增长图.png" alt="{esc(source)} 数据增长图">
      </article>
    </section>
    <section class="section-grid">
    ...
    <section id="appendix" class="panel">
      ...
      <details>
        ...
        <p>模块画像.json、模块画像_关注点.csv、模块画像_思考模式.csv、模块画像_数据增长_按月.csv、模块画像_数据增长图.png。</p>
      </details>
    </section>
    """
    return report_shell(f"{source} 模块画像", f"{source} 数据模块的行为信号、增长和关注点分析。", body)
...
def render_integrated_html(
    ...
    <section id="summary" class="panel">
      <h2>Executive Summary</h2>
      <p class="lede">统合画像把 Google、GPT、Agent 三类数据拼成一条个人数据链路：外部输入进入思考层，再进入本机 Agent 执行和长期能力沉淀。</p>
      <ul class="insight-list">
        <li><strong>数据流向已经闭环：</strong>原始数据进入结构化层，生成分析画像，再汇总到统合 SQLite。</li>
    ...
    <section class="section-grid">
      <article id="growth" class="panel">
        ...
        <img class="chart-img" src="统合画像_数据增长图.png" alt="统合数据增长图">
      </article>
      <article id="attention" class="panel">
        <h2>关注点</h2>
    ...
    <section id="flow" class="panel">
      <h2>数据流向</h2>
      <p class="lede">每个模块都遵循原始数据、结构化数据、分析数据、统合模块的证据链。</p>
      {''.join(flow_html)}
    </section>
    <section id="appendix" class="panel">
      ...
      <ul class="insight-list">
        ...
        <li>后续重新导出数据后，先重建统合数据库，再重跑画像脚本即可刷新本报告。</li>
      </ul>
      <details>
        <summary>Evidence files</summary>
        <p>personal_system.sqlite、统合画像.json、统合画像_数据流向.csv、统合画像_数据增长_按月.csv、统合画像_关注点.csv、统合画像_各模块关注点.csv、统合画像_个人思考模式.csv、统合画像_数据增长图.png。</p>
      </details>
    </section>
    """
    return report_shell("个人数据统合画像", "从 Google、GPT、Agent 三类本地数据生成的个人系统级分析报告。", body)
...
当前个人数据系统已经从单点数据分析升级为三源统合画像：Google 代表外部信息输入，GPT 代表显性提问和内容生产，Agent 代表本机执行系统与长期能力沉淀。三者合在一起，形成“输入 - 思考 - 执行 - 记忆”的闭环。
