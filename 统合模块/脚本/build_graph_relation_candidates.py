...
输入:
- 统合模块/分析数据/ai_context/conversation_summaries.json
...
用法:
  python 统合模块\脚本\build_graph_relation_candidates.py --dry-run
  python 统合模块\脚本\build_graph_relation_candidates.py --write --limit 100
"""
...
SUMMARIES_JSON = ROOT / '统合模块' / '分析数据' / 'ai_context' / 'conversation_summaries.json'
SQLITE_DB = ROOT / '统合模块' / 'SQLite数据库' / 'personal_system.sqlite'
COLLECTION_NAME = 'conversation_turns'
DEFAULT_TOP_K = 8
DEFAULT_LIMIT = 0
MIN_SEMANTIC_SCORE = 0.78
SAMPLE_REPORT = ROOT / '统合模块' / '分析数据' / 'ai_context' / 'graph_relation_candidates_report.json'
...
def preflight_vector_sync(turn_map: dict[str, dict]) -> dict:
    ...
    if not coll_info:
        raise RuntimeError(
            'conversation_turns collection 不存在; 先运行 "python 统合模块/脚本/build_conversation_vector_store.py --write"'
        )
    ...
    if actual_count != expected_count:
        raise RuntimeError(
            ...
            '先重跑 "python 统合模块/脚本/build_conversation_vector_store.py --write"'
        )
