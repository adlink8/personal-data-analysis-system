...
# 新管道（pk-sync conversations）走 ~/.agentsview/sessions.db；sync.py 仅读取本模块
# 历史输出文件 conversation_summaries.json（当前已不存在，sync 已容错），不 import 本模块。
...
from personal_knowledge.core.llm import make_llm_client, _chat_with_retry, MAX_RETRY

ROOT = Path(__file__).resolve().parents[4]
AGENT_DB = ROOT / "Agent" / "structured" / "db" / "agent_data.sqlite"
OUT_JSON = ROOT / "integration" / "analysis" / "ai_context" / "conversation_summaries.json"
