...
# -----------------------------------------------------------------------------
# Phase 20 迁移已删除 GPT/ 目录，本模块引用的
# ROOT/"GPT"/"structured"/"db"/"chatgpt_data.db" 已不存在，运行即报错。
...
from personal_knowledge.application.conversation import summary as agent_summary_mod
...
def run(dry_run: bool, write: bool, limit: int | None, max_chars: int,
    ...
    prepared = []
    for idx, row in enumerate(conv_rows, 1):
        conv_id = row[0]
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--order", choices=["conversation_id", "latest"], default="conversation_id",
                   help="conversation 处理顺序:conversation_id 或 latest(按最近消息优先)")
    ...
    return run(args.dry_run, args.write, args.limit, args.max_chars, args.resume, workers, args.order)
