...
# 共享工具函数和规则(Phase 13 迁移：消除本地重复定义)
sys.path.insert(0, str(Path(__file__).resolve().parent))
from personal_knowledge.core.common import sha256_text, norm, short, event_id, entity_id, extract_domain, extract_tools, write_csv  # noqa: E402
from personal_knowledge.core.rules import TOOL_NAMES, TOPIC_RULES  # noqa: E402
...
# Phase 13.5: canonical conversation store（会话证据层）
# 注意：build_integrated_system 的 Agent 数据来自 Phase 02 的结构化 dataset
#（sessions/skills/memories/session_messages），不是 canonical conversation store。
# canonical store 是会话证据入口，Phase 14 knowledge unit 直接消费它。
# build_integrated_system 的 canonical 接入 deferred 到后续阶段。
...
def connect_event_entities(event: dict, entities: dict[str, dict], event_entities: list[dict]) -> None:
    ...
    for tool in extract_tools(text, TOOL_NAMES):
