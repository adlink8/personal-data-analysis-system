...
describe('EvidencePage（/evidence）', () => {
  ...
  it('MCP 服务不可达（ports.mcp.up=false）：三个 Widget 均显示非空 recovery card，而非空白 iframe', () => {
    ...
    const cards = screen.getAllByText('诊断集成当前不可达');
  ...
  it('system status 查询失败时仍非空：显示"无法确认"提示，且不阻止 Widget 尝试加载', () => {
    ...
    // 未知态（非"已确认不可达"）：仍尝试渲染 iframe，而不是直接判定不可达
  ...
  it('iframe 加载超时（受控超时判断）显示非空提示；随后 onLoad 触发后清除提示（不作为 authoritative success 标注）', async () => {
    vi.useFakeTimers();
    currentQuery = { isPending: false, isError: false, data: withMcpUp(true), refetch: vi.fn() };
    render(<EvidencePage />);
    ...
    // 加载完成不等于"权威成功"：页面不会出现任何"成功"/"已验证"字样绑定给 Widget
