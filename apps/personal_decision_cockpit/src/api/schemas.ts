...
const PersonalAssertionSchema = z
  .object({
    ...
    status: z.string().nullable(),
    confidence: z.union([z.number(), z.string()]).nullable(),
    current_assertion_id: z.string().nullish(),
...
// 后端可能微调字段名（如 valid_to ↔ valid_at）：全部 nullish，缺字段由页面显式"未提供"
const ExternalFactSchema = z
  .object({
    fact_id: z.string().nullish(),
    ...
    source_id: z.string().nullish(),
    source_quality: z.union([z.number(), z.string()]).nullish(), // 真实数据为数值（如 0.99）
    lifecycle: z.string().nullish(),
    conflict: z.boolean().nullish(),
  })
  .passthrough();
