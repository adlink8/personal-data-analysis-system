...
const VERTEX_PROVIDER_ENABLED = ["vertex_google", "vertex", "gemini"].includes(CONFIGURED_MODE);
const REAL_PROVIDER_ENABLED = ["aliyun", "dashscope"].includes(CONFIGURED_MODE) || VERTEX_PROVIDER_ENABLED;
const REAL_MODEL = String(process.env.PI_PROVIDER_MODEL ?? (VERTEX_PROVIDER_ENABLED ? process.env.PERSONAL_DATA_VERTEX_MODEL : PERSISTED_CONFIG.model) ?? (VERTEX_PROVIDER_ENABLED ? "gemini-3.5-flash" : "deepseek-v4-flash-0731")).trim() || (VERTEX_PROVIDER_ENABLED ? "gemini-3.5-flash" : "deepseek-v4-flash-0731");
...
const provider = VERTEX_PROVIDER_ENABLED ? "vertex_google" : REAL_PROVIDER_ENABLED ? "dashscope" : "replay";
const model = REAL_PROVIDER_ENABLED ? REAL_MODEL : "replay-v1";
const currency = REAL_PROVIDER_ENABLED ? (VERTEX_PROVIDER_ENABLED ? "USD" : (PERSISTED_CONFIG.currency || "CNY")) : "CNY";
