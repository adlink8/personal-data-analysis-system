...
function makeFakeFetch() {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (parsed.pathname === "/search/semantic") {
      ...
      return jsonResponse({ ok: true, data: [{ event_id: "evt_1", title: `Result for ${body.query}` }] });
    }
...
test("data tools call /data REST endpoints and return structuredContent", async () => {
  ...
  const exported = await callTool("data_export_query", { query: "Codex", format: "jsonl" }, { fetchImpl, restBaseUrl: "http://rest.test" });
