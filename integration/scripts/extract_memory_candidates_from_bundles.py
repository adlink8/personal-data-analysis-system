...
import build_conversation_summary as llm_mod
...
def write_candidates(con: sqlite3.Connection, candidates: list[dict[str, Any]]) -> int:
    ...
    with closing(sqlite3.connect(db_path)) as con:
        ...
        if runtime.llm_status != "live_api_key_present":
            ...
            for bundle in bundles:
                ...
                for claim in normalized_claims:
                    row = build_candidate_row(claim, bundle, created_at=generated_at)
                    if row is None:
                ...
                bundle_results.append(
                    {
                        "bundle_id": bundle["bundle_id"],
                        "status": "ok" if written_for_bundle else "rejected",
                        "candidate_count": written_for_bundle,
        ...
        if write and runtime.llm_status == "live_api_key_present":
            ...
            if not failed_without_replacement:
                written_count = write_candidates(con, candidates)
