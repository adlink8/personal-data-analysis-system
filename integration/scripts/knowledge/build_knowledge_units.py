...
from knowledge.knowledge_unit_pipeline import RunManifest, StagingPublisher  # noqa: E402

# Vertex AI 配置
GCP_PROJECT = "project-c5cbd608-1b00-454e-80f"
GCP_SDK_PY = "C:/Users/li/google-cloud-sdk/lib/gcloud.py"
VERTEX_MODEL = "gemini-3.5-flash"
PROMPT_VERSION = "v1"

PROMPT_PATH = _THIS_DIR.parent / "prompts" / "knowledge_unit_extractor" / "v1_main.md"
...
def _get_gcloud_token() -> str:
    if os.path.exists(GCP_SDK_PY):
        env = dict(os.environ)
        env["CLOUDSDK_ROOT_DIR"] = "C:/Users/li/google-cloud-sdk"
        env["CLOUDSDK_CORE_DISABLE_PROMPTS"] = "1"
        r = subprocess.run([sys.executable, GCP_SDK_PY, "auth", "print-access-token"],
                           capture_output=True, text=True, env=env)
        if r.returncode == 0:
            return r.stdout.strip()
    r = subprocess.run(["gcloud", "auth", "print-access-token"], capture_output=True, text=True)
    if r.returncode != 0:
        raise RuntimeError(f"获取 token 失败: {r.stderr.strip()}")
    return r.stdout.strip()
...
def call_llm(system_prompt: str, user_content: str) -> dict:
    ...
    url = (f"https://aiplatform.googleapis.com/v1/projects/{GCP_PROJECT}"
           f"/locations/us-central1/publishers/google/models/{VERTEX_MODEL}:generateContent")
