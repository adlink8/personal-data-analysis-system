...
用法:
  python integration\scripts\audit_memory_experiments.py
...
DUCKDB_DB = ROOT / "integration" / "db" / "conversation_graph.duckdb"
...
def scan_duckdb_tables() -> dict:
    ...
    if duckdb is None:
        ...
        report["actions"].append(
            "DuckDB Python 模块不可用；先恢复 duckdb 运行环境，再重跑 `python integration/scripts/audit_memory_experiments.py --write`。"
        )
        return report
...
def scan_chroma_collections() -> dict:
    ...
    try:
        ...
        report["actions"].append(
            "Chroma 不可用；先恢复 127.0.0.1:8001 服务，再重跑 `python integration/scripts/audit_memory_experiments.py --write`。"
        )
