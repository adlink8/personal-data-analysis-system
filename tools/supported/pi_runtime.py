...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("command", choices=["inspect", "prepare", "confirm", "downgrade"], default="inspect", nargs="?"); parser.add_argument("--database", default="var/db/pi_runtime_activation.sqlite"); parser.add_argument("--target", default="legacy"); parser.add_argument("--evidence-checksum", default=""); parser.add_argument("--preview", default=""); parser.add_argument("--phrase", default=""); parser.add_argument("--idempotency-key", default=""); args = parser.parse_args()
    runtime = RuntimeActivation(args.database)
    try:
        if args.command == "inspect": result = runtime.current()
        elif args.command == "prepare": result = runtime.prepare(args.target, evidence_checksum=args.evidence_checksum)
