...
def test_migrate_adds_columns(tmp_path: Path) -> None:
    ...
    from sync_memory_lifecycle import migrate
...
def test_preview_no_db_diff(tmp_path: Path) -> None:
    ...
    from sync_memory_lifecycle import build_preview, migrate
...
def test_write_links_matching(tmp_path: Path) -> None:
    ...
    from sync_memory_lifecycle import migrate, build_preview, apply_write
    migrate(db, write=True)
    preview = build_preview(db)
...
def test_write_hash_mismatch_rejected(tmp_path: Path) -> None:
    ...
    from sync_memory_lifecycle import migrate, apply_write
    migrate(db, write=True)
...
def test_preview_hash_stable(tmp_path: Path) -> None:
    ...
    from sync_memory_lifecycle import migrate, build_preview
    migrate(db, write=True)
    p1 = build_preview(db)
    p2 = build_preview(db)
