...
EVAL_DIR = _ROOT / "integration" / "evals" / "knowledge_units"
...
def test_synthetic_cases_schema() -> None:
    ...
    cases = _load_jsonl(EVAL_DIR / "synthetic_cases.jsonl")
    assert len(cases) >= 10
...
def test_dev_dataset_schema() -> None:
    ...
    cases = _load_jsonl(EVAL_DIR / "dev_queries.private.jsonl")
    assert len(cases) == 20
    for c in cases:
        assert c["split"] == "dev"
...
def test_frozen_dataset_schema() -> None:
    ...
    cases = _load_jsonl(EVAL_DIR / "frozen_test_queries.private.jsonl")
    assert len(cases) == 20
    for c in cases:
...
def test_no_dev_frozen_leak() -> None:
    ...
    dev = _load_jsonl(EVAL_DIR / "dev_queries.private.jsonl")
    frozen = _load_jsonl(EVAL_DIR / "frozen_test_queries.private.jsonl")
    dev_refs = set()
    for c in dev:
...
def test_merge_positive_pairs() -> None:
    ...
    pairs = _load_jsonl(EVAL_DIR / "merge_positive_pairs.private.jsonl")
    assert len(pairs) == 20
    for p in pairs:
...
def test_hard_negative_pairs() -> None:
    ...
    pairs = _load_jsonl(EVAL_DIR / "hard_negative_pairs.private.jsonl")
    assert len(pairs) == 20
    for p in pairs:
...
def test_readme_exists() -> None:
    """README 文档存在。"""
    assert (EVAL_DIR / "README.md").exists()
