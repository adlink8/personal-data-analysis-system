...
from build_agentsview_normalized import (  # noqa: E402
    NORMALIZED_SCHEMA,
    NormalizationStats,
    RevisionGateError,
    build_normalized,
...
def _make_source_fixture(dest: Path) -> Path:
    ...
    cur.execute(
        "INSERT INTO messages (session_id, ordinal, role, content, timestamp) "
        "VALUES ('s1',3,'user','my key is sk-abcdefghijklmnopqrstuvwxyz123456 here','2026-01-01T00:00:03Z')"
    )
    # s1 注入含邮箱的 message
    cur.execute(
        "INSERT INTO messages (session_id, ordinal, role, content, timestamp) "
    ...
    cur.execute(
        "INSERT INTO messages (session_id, ordinal, role, content, timestamp) "
        "VALUES ('s2',1,'user','this has a secret sk-leaked12345678901234567890','2026-01-02T00:00:01Z')"
    )
