...
_REST_PORT = 8000
_SUPERVISOR_STATE_PATH = Path(__file__).resolve().parents[3] / "ops" / "state" / "agent-stack.json"
...
class CockpitProjectionService:
    ...
    def _system_status_get(self, **_params: Any) -> dict[str, Any]:
        ...
        knowledge = sections.get("knowledge") or {}
        sections["observations"] = self._runtime_observations(sections, generated_at)
        sections["supervisor_state"] = self._supervisor_state_section(generated_at)
        snapshot_bindings = {
            "personal": None,
            "external": None,
            "serving": knowledge.get("serving_snapshot_id"),
