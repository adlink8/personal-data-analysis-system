  ...
  return (
    <div className="model-provider-panel">
      <aside className="provider-list" aria-label="模型提供商">
        <div className="provider-list-heading"><strong>已配置提供商</strong><small>Pi Provider Adapter</small></div>
        {providerOrder.map((id) => (
          <button type="button" className={selected === id ? "selected" : ""} aria-label={presets[id].name} onClick={() => selectProvider(id)} key={id}>
            <span className="provider-cube" aria-hidden="true" /><span><strong>{presets[id].name}</strong><small>{id === "compatible" ? "本地 / 自定义" : "内置适配器"}</small></span><i aria-label="已配置" />
          </button>
        ))}
      ...
      <div className="provider-form">
        <div className="provider-form-heading"><div><h3>{selected === "custom" ? "添加模型提供商" : `配置 ${name}`}</h3><p>提供商配置由 Pi Kernel 路由，聊天时可按任务选择。</p></div><span className="provider-draft">原型</span></div>
        <div className="provider-fields">
          ...
          <label><span>API Key</span><input aria-label="API Key" type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="仅保存到系统凭据存储" autoComplete="off" /></label>
          ...
          <fieldset className="capability-picker"><legend>声明能力</legend><p>用于路由候选，真实能力仍由连接测试和 Provider receipt 确认。</p><div>{allCapabilities.map((capability) => <label key={capability}><input type="checkbox" checked={capabilities.includes(capability)} onChange={() => toggleCapability(capability)} /><span>{capability}</span></label>)}</div></fieldset>
        </div>
        ...
        <div className="provider-secret-note"><strong>凭据边界</strong><span>正式版 API Key 仅经 Main Process 写入 Windows Credential Manager / DPAPI；Renderer、日志和 SQLite 不保存明文。</span></div>
      </div>
    </div>
  );
}
