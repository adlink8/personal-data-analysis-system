...
describe("selected C composition", () => {
  it("opens directly to conversation and keeps secondary surfaces closed", () => {
    render(<App />);
    expect(screen.getByRole("navigation", { name: "项目与会话" })).toBeVisible();
  ...
  it("opens history and evidence in a docked context sidebar", () => {
    render(<App />);
  ...
  it("opens settings from the bottom rail and returns to the conversation", () => {
    ...
    expect(screen.queryByRole("navigation", { name: "项目与会话" })).not.toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "设置" })).not.toBeInTheDocument();
    expect(screen.getByLabelText("启动时打开")).toBeVisible();
    expect(screen.queryByRole("switch", { name: "AgentsView 新鲜度检查" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("link", { name: "对话与隐私" }));
    expect(screen.getByRole("switch", { name: "AgentsView 新鲜度检查" })).toBeVisible();
    expect(screen.queryByLabelText("启动时打开")).not.toBeInTheDocument();
  ...
  it("keeps settings controls local to the visual spike", () => {
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "设置" }));
    fireEvent.click(screen.getByRole("link", { name: "主动提醒" }));
    ...
    expect(screen.queryByText("原型预览：设置只保存在当前页面，不写入真实 DesktopBridge。")).not.toBeInTheDocument();
  });

  it("reveals custom window controls only for the custom window form", () => {
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "设置" }));
    fireEvent.click(screen.getByRole("link", { name: "界面与窗口" }));

    expect(screen.getByRole("radio", { name: "标准工作台" })).toBeVisible();
  ...
  it("lets the user edit interface code and apply it only to an isolated preview", () => {
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "设置" }));
    fireEvent.click(screen.getByRole("link", { name: "代码定制" }));

    expect(screen.queryByRole("heading", { name: "代码定制" })).not.toBeInTheDocument();
    ...
    expect(screen.getByText("预览已更新")).toBeVisible();
    expect(screen.queryByText("JavaScript / TypeScript 不在主 Renderer 中执行。")).not.toBeInTheDocument();
  });

  it("keeps a separate code draft for every interface region", () => {
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "设置" }));
    fireEvent.click(screen.getByRole("link", { name: "代码定制" }));

    const editor = screen.getByRole("textbox", { name: "界面代码编辑器" });
    fireEvent.click(screen.getByRole("tab", { name: "左侧栏" }));
    fireEvent.change(editor, { target: { value: ".codex-sidebar { opacity: .9; }" } });
    fireEvent.click(screen.getByRole("tab", { name: "输入框" }));
    expect((editor as HTMLTextAreaElement).value).toContain(".composer");
    fireEvent.click(screen.getByRole("tab", { name: "左侧栏" }));
    expect(editor).toHaveValue(".codex-sidebar { opacity: .9; }");
  });
  ...
  it("rejects network-loading or script syntax before the custom code preview", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "应用到隔离预览" }));
    expect(screen.getByText("检测到网络资源或脚本语法。")).toBeVisible();
  });
  ...
  it("groups settings like ZCode and exposes model provider configuration", () => {
    ...
    fireEvent.click(screen.getByRole("link", { name: "模型接入" }));
